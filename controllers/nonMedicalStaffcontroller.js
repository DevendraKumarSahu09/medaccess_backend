...
export const addNonMedicalStaff = async (req, res) => {
    try {
        ...
        const existingStaff = await NonMedicalStaff.findOne({ email });
        if (existingStaff) {
...
export const updateNonMedicalStaff = async (req, res) => {
    try {
        ...
        // If updating email, check if it's already in use (except by the current staff)
        if (req.body.email) {
            const existingStaff = await NonMedicalStaff.findOne({ 
